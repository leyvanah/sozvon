...
type Description struct {
	...
	// Whether this is an operator-room child (hub/<slug>).  Runtime only,
	// derived when the parent hub has OperatorRoom set; never persisted.
	// Such a room forces the lobby (knock-and-approve) and does not let a
	// token bypass the waiting room, so the operator vets every client.
	isOperatorRoomChild bool `json:"-"`
	...
	// Whether non-op users are placed in a waiting room (lobby) and must
	// be admitted by an operator before joining.  A lobby group is always
	// gated this way whenever an operator is present, independently of the
	// group's locked state (so it survives a restart and needs no autolock).
	Lobby bool `json:"lobby,omitempty"`

	// Whether this group runs in end-to-end encrypted mode.  When set, the
	// web client encrypts media in the browser with a key the server never
	// sees (ephemeral ECDH authenticated by an emoji SAS), so the SFU only
	// ever forwards ciphertext.  This is advertised to clients in the group
	// status; the encryption itself is performed entirely in the browser.
	E2EE bool `json:"e2ee,omitempty"`

	// Whether an end-to-end encrypted group refuses connections that cannot
	// be encrypted.  When set, the group is limited to two participants (the
	// most the pairwise E2EE covers): the server turns away anyone who would
	// be the third, and clients that cannot encrypt do not publish media, so
	// the call can never silently downgrade to cleartext.  Only meaningful
	// together with E2EE.
	RequireE2EE bool `json:"require-e2ee,omitempty"`

	// Whether this group is an operator hub.  When set, an operator who
	// logs in lands on a dashboard to manage per-client invite links, each
	// scoped to a subgroup hub/<slug>.  Only meaningful on a top-level
	// group; it is cleared on the subgroups themselves (see readDescription).
	OperatorRoom bool `json:"operator-room,omitempty"`
...
func readDescription(name string, allowSubgroups bool) (*Description, error) {
	...
	if isSubgroup {
		if !desc.AutoSubgroups && !desc.OperatorRoom {
			return nil, os.ErrNotExist
		}
		desc.isSubgroup = true
		desc.Public = false
		desc.Description = ""
		if desc.OperatorRoom {
			// A subgroup of an operator hub is a per-client room:
			// force the lobby so the operator admits every client,
			// and clear OperatorRoom so the child is not itself a
			// hub and its status never advertises the dashboard.
			// desc is a fresh per-read copy, so none of this is
			// written back to the parent's JSON file on disk.
			desc.isOperatorRoomChild = true
			desc.Lobby = true
			desc.OperatorRoom = false
		}
	}
...
// OperatorHubName returns the name of the operator-room hub that is served at
// the site root, or "" if there is none.  It is the first top-level group whose
// description has operator-room set (WalkDir visits entries in lexical order).
func OperatorHubName() string {
	names, err := GetDescriptionNames()
	if err != nil {
		return ""
	}
	for _, name := range names {
		if strings.ContainsRune(name, '/') {
			continue // a subgroup is never a hub
		}
