...
import (
	...
	"github.com/leyvanah/sozvon/stats"
...
func checkAdmin(w http.ResponseWriter, r *http.Request, groupname string) bool {
	...
	if !ok {
		if hasAuth || creds.Token != "" {
			// escalating delay slows password guessing (Sozvon);
			// a credential-less request (the browser's initial
			// probe) is not counted as a failure
			authlimit.Failure(key, "admin api")
		}
