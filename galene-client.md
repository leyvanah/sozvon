...
serverConnection.onjoined = function(kind, group, perms, status, data, error, message) {
    ...
    case 'fail':
        ...
        break;
    // The following three kinds are Sozvon extensions, sent only for
    // groups configured with "lobby": true (see galene-protocol.md).
    case 'knock':
        // placed in the waiting room; show a "waiting for the operator"
        // screen until a 'rejoin' or 'deny' message arrives
        break;
    case 'rejoin':
        // an operator admitted us: resend the join message, which will
        // now succeed
        this.join(group, 'join', username, password);
        break;
    case 'deny':
        // an operator denied us; display a friendly message
        break;
}
```
