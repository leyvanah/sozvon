...
The following fields are **Sozvon extensions**, not present in upstream
Galène:

 - `lobby`: if true, non-operator users are placed in a waiting room and
   must be admitted or denied by an operator before joining.  A lobby group
   is gated this way whenever an operator is present, independently of the
   group's locked state, so it behaves like a private room across server
   restarts without relying on `autolock`.  Invite tokens bypass the lobby.
   See *Lobby and operator rooms* below;

 - `e2ee`: if true, this group supports end-to-end encryption between two
   participants: media is encrypted in the browser with a key the server
   never sees, so the SFU only ever forwards ciphertext.  See *End-to-end
   encryption* below;

 - `require-e2ee`: only meaningful together with `e2ee`.  If true, the
   group is limited to two participants, and a client that cannot encrypt
   refuses to publish media instead of silently falling back to cleartext;

 - `operator-room`: if true, this group is an operator hub: an operator who
   logs in lands on a dashboard instead of the call UI, and issues personal
   invite links per client, each opening into its own gated child room
   `hub/<slug>`.  Only meaningful on a top-level group.  See *Lobby and
   operator rooms* below.

### Lobby and operator rooms

A `lobby` group asks every non-operator joiner to wait for an operator to
admit or deny them:
...
An `operator-room` group turns into a dashboard for issuing personal,
per-client invite links instead of an ordinary call.  It normally has no
`wildcard-user` and is not `public`; child rooms `<hub>/<slug>` are created
on demand as clients knock, inherit the hub's codecs, keys and `e2ee`
setting, and disappear once empty:
...
Setting `e2ee` (optionally together with `require-e2ee`) turns on
per-group, browser-side encryption for two-participant calls: an ephemeral
ECDH key agreement authenticated by a 5-emoji Short Authentication String
that the participants compare aloud, then AES-256-GCM on each media frame.
The server never sees the keys and only ever forwards ciphertext.  See
[FORK-CHANGES.md](FORK-CHANGES.md) for the full design, including scope
(two participants, VP8/Opus only), fallback behaviour, and encrypted chat.
