...
use a matching invite token, a non-operator `join` does not immediately
succeed or fail: the client is placed in a waiting room and receives
a `joined` message of kind `knock` instead of `join` or `fail`:
...
An operator resolves the request by sending a `useraction` message of kind
`admit` or `deny`, with `dest` set to the id of the waiting client:
...
The waiting client then receives a second `joined` message: kind `rejoin`
on admission — the client should immediately resend its `join` message,
which will now succeed — or kind `deny` on refusal.

While a client is knocking, every operator connected to the group receives
`user` messages of kind `knock` (a new or still-pending request) and
...
*(Sozvon extension.)* If the group has `"e2ee": true`, the client library
performs an additional key-agreement handshake (ephemeral ECDH,
authenticated by a 5-emoji Short Authentication String) and encrypts each
media frame with AES-256-GCM, entirely in the browser.  The handshake and
SAS-comparison state ride as opaque payloads inside ordinary `usermessage`
messages; the server does not parse or interpret them, and none of the
wire-level messages defined above are otherwise affected.  See
[FORK-CHANGES.md](FORK-CHANGES.md) for the cryptographic design.
