...
> **For Sozvon:** this is upstream Galène's installation guide and still
> applies almost verbatim — substitute `sozvon` for `galene` in the binary
> name, the dedicated server user, and paths like `/home/galene`.  See the
> [README](README.md) for the Sozvon-specific build command and for
> `-letsencrypt`, a built-in alternative to the certbot/reverse-proxy setup
> below; a ready-made systemd unit is at
> [contrib/systemd/sozvon.service](contrib/systemd/sozvon.service) instead of
> the example unit further down.
