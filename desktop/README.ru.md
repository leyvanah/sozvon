...
На Windows без права создавать символические ссылки electron-builder падает
при распаковке инструментов `winCodeSign` (в архиве лежат симлинки для
macOS). Один раз распакуйте архив из
`%LOCALAPPDATA%electron-builderCachewinCodeSign` в папку
`winCodeSign-2.6.0` рядом, не обращая внимания на две ошибки о симлинках, —
и сборка проходит.

## Конфигурация

Хранится в `%APPDATA%/sozvon-desktop/config.json` и редактируется через
стартовый экран и меню в трее. Поля:

- `servers` — сохранённые серверы, по одному объекту на сервер:
  `{"url": "…", "name": "Клиника", "hub": "…", "lastGroup": "…", "rooms": ["…"]}`. Список
  ...
  сервер **добавляется**, а не затирает предыдущий. `hub` — операторская
  этого сервера, как её сообщил клиент.
...
- `theme` — `system`, `light` или `dark`, как выбрано в веб-клиенте.
- `minimizeToTray` — крестик прячет окно в трей (по умолчанию включено).
- `autoStart` — запускаться вместе с Windows, сразу в трей (по умолчанию
  выключено).
...
- `allowInsecureCerts` — аварийный обход: принимать любой сертификат для
  ...
  отладки. При изменении приложение перезапускается само.
