...
func (token *Stateful) Check(host, group string) (string, []string, error) {
	...
	now := time.Now()
	// A nil expiry means the token never expires (used for perpetual
	// operator-room invitation links).  sortStateful and the expired-token
	// cleanup already treat a nil expiry as non-expiring.
...
// sortStateful orders tokens by ascending expiry, non-expiring last.
func sortStateful(a []*Stateful) {
...
// called locked
func (state *state) listSubtree(group string) ([]*Stateful, string, error) {
	_, err := state.load()
	if err != nil {
		return nil, "", err
	}
	...
	prefix := group + "/"
	for _, t := range state.tokens {
		if t.Group != group && !strings.HasPrefix(t.Group, prefix) {
			continue
		}
		a = append(a, t)
	}
...
func (state *state) ListSubtree(group string) ([]*Stateful, string, error) {
	state.mu.Lock()
	defer state.mu.Unlock()
	return state.listSubtree(group)
}

// ListSubtree returns every token whose group is group or a subgroup of
// it (group/...).  It backs the operator-room dashboard, which lists the
// hub's own tokens together with those of its per-client child rooms.
func ListSubtree(group string) ([]*Stateful, string, error) {
	return tokens.ListSubtree(group)
}
